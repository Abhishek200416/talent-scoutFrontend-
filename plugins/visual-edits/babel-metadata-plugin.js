...
function fileExportHasPortals({
  ...
  }); } catch(e) { /* ignore traverse errors */ }
...
function usageIsCompositePortal({
  ...
  if (binding && binding.path) {
      ...
      JSXOpeningElement(op) {
        ...
        if (/^[A-Z]/.test(name || "")) {
          ...
          if (innerBinding && innerBinding.path) {
            try {
              innerBinding.path.traverse(jsxVisitor);
            } catch (e) {
    ...
    binding.path.traverse(jsxVisitor);
    if (hit) return true;
  }
    ...
    for (const [absPath, cached] of FILE_AST_CACHE) {
      ...
      traverse(ast, {
        ImportDeclaration(importPath) {
          ...
          try { importPath.parentPath.parentPath.traverse({
            JSXOpeningElement(jsxPath) {
              if (result) return;
